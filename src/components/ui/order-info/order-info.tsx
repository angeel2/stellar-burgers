...
import { TOrderInfoUIProps } from './type';
import { OrderStatus } from '@components';

export const OrderInfoUI: FC<TOrderInfoUIProps> = memo(({ orderInfo }) => (
  <div className={styles.wrap}>
    ...
    <ul className={`${styles.list} mb-8`}>
      {Object.values(orderInfo.ingredientsInfo).map(
        (ingredientWithCount, index) => (
          <li className={`pb-4 pr-6 ${styles.item}`} key={index}>
            <div className={styles.img_wrap}>
              <div className={styles.border}>
                <img
                  className={styles.img}
                  src={ingredientWithCount.image_mobile}
            ...
            >
              {ingredientWithCount.count} x {ingredientWithCount.price}
            </span>
